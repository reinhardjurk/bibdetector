...
# --- Configuration ---
VIDEO_PATH = 'test.long.m4v'  # Default video path
...
TIME_OFFSET = 0  # Default time offset in seconds
...
# Parse CLI arguments for time offset and video filename
if len(sys.argv) > 1:
    time_offset_str = sys.argv[1]
    if len(time_offset_str) == 6 and time_offset_str.isdigit():
        minutes = int(time_offset_str[:2])
        seconds = int(time_offset_str[2:4])
        hundredths = int(time_offset_str[4:6])
        TIME_OFFSET = minutes * 60 + seconds + hundredths / 100.0
        print(f"Time offset set to {TIME_OFFSET} seconds (mmssxx: {time_offset_str})")
    else:
        print("Invalid time offset format. Use mmssxx (e.g., 023050 for 2m 30s 50ms). Using default offset of 0.")

if len(sys.argv) > 2:
    VIDEO_PATH = sys.argv[2]
    print(f"Video file set to: {VIDEO_PATH}")
    ...
    <body>
        ...
        <table>
            <tr>
                <th>Bib Number</th>
                <th>Time (hh:mm:ss)</th>
                <th>Images (Close-up & Complete with Time)</th>
                <th>Playback</th>
            </tr>
    """)

def append_to_html(number, timestamp_str, bib_closeup_filename, m3u_filename, complete_picture_filename):
    with open(HTML_FILE, 'a', encoding='utf-8') as f:
        f.write(f"""
            <tr>
                ...
                <td>{timestamp_str}</td>
                <td>
                    <strong>Close-up Bib:</strong><br>
                    <img src="{OUTPUT_DIR}/{bib_closeup_filename}" alt="Bib {number} Close-up" style="max-width: 300px; border: 2px solid green;">
                    <br><br>
                    <strong>Complete Picture with Time:</strong><br>
                    <img src="{OUTPUT_DIR}/{complete_picture_filename}" alt="Bib {number} Complete" style="max-width: 500px; border: 2px solid blue;">
                </td>
...
while cap.isOpened():
    ...
    current_timestamp_str = str(timedelta(seconds=current_time_sec + TIME_OFFSET))[:-3]
    ...
    for t_id in tracks_to_process:
        ...
        if len(sightings) >= MIN_CONSECUTIVE_DETECTIONS:
            ...
            if max_consecutive >= MIN_CONSECUTIVE_DETECTIONS and best_num not in globally_completed_bibs:
                ...
                bib_closeup_filename = f"bib_closeup_{final_number}.jpg"
                ...
                complete_picture_filename = f"complete_{final_number}.jpg"
                complete_frame = best_frame.copy()
                
                # Draw bib detection
                cv2.rectangle(complete_frame, p1, p2, (0, 255, 0), 3)
                cv2.rectangle(complete_frame, (p1[0], p1[1] - 40), (p1[0] + 150, p1[1]), (0, 255, 0), -1)
                cv2.putText(complete_frame, f"Bib: {final_number}", (p1[0] + 5, p1[1] - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 2)
                
                # Display calculated time prominently in lower left corner
                display_time = best_time_sec + TIME_OFFSET
                time_text = str(timedelta(seconds=display_time))[:-3]
                cv2.putText(complete_frame, time_text, (50, h_img - 50),
                            cv2.FONT_HERSHEY_SIMPLEX, 3.0, (255, 255, 255), 8)  # Very large, bold, white text
                ...
                append_to_html(final_number, best_timestamp, bib_closeup_filename, m3u_filename, complete_picture_filename)
...
video_length_sec = frame_idx / fps

cap.release()

print(f"Total unique runners identified: {len(globally_completed_bibs)}")
print(f"Video length: {str(timedelta(seconds=video_length_sec))[:-3]}")
print(f"Processing time: {str(timedelta(seconds=processing_time_sec))[:-3]}")
print(f"Processing speed: {video_length_sec / processing_time_sec:.2f}x real-time")
print(f"Analysis complete. Open 'race_results.html' in your browser.")
